...
| 항목 | 값 |
|---|---|
| 전체 시스템 수 | 11 (10 게임 시스템 + 턴 시스템) |
| P0 (코어) | 건설, 인력 관리, 웨이브 방어, 탐사/원정 |
| P1 (중요) | 인카운터/이벤트, 기술 트리, 정책/법률, 희망/불만, 방벽 확장 |
| P2 (후순위) | 시대 전환/메타 진행 |
| 인프라 | 턴 시스템, 자원 시스템 |
| 현재 마일스톤 | M1: 프로토타입 |
...
| SF | 서브피처 | 크기 | 상태 |
|---|---|---|---|
| SF-01 | 데이터 모델 (노드, 맵 SO) | M | 미시작 |
| SF-02 | ExplorationManager 코어 (맵, 안개, 이동) | XL | 미시작 |
| SF-03 | 인카운터/전투 연동 | M | 미시작 |
| SF-04 | 턴/자원/인력 연동 | M | 미시작 |
| SF-05 | 탐사 UI + 테스트 씬 | L | 미시작 |

---

### 기술 트리 시스템 (tech-tree-system)
- **GDD**: Docs/GDD/systems/tech-tree-system.md
- **브랜치**: feature/tech-tree-system
- **우선순위**: P1
- **상태**: GDD작성

| SF | 서브피처 | 크기 | 상태 |
|---|---|---|---|
| SF-01 | 데이터 모델 (노드 SO, 카테고리) | M | 미시작 |
| SF-02 | TechTreeManager 코어 | L | 미시작 |
| SF-03 | 건설/방어/자원 연동 | M | 미시작 |
| SF-04 | 턴 시스템 연동 (연구 진행도) | M | 미시작 |
| SF-05 | 기술 트리 UI + 테스트 씬 | L | 미시작 |

---

### 정책/법률 시스템 (policy-system)
- **GDD**: Docs/GDD/systems/policy-system.md
- **브랜치**: feature/policy-system
- **우선순위**: P1
- **상태**: GDD작성

| SF | 서브피처 | 크기 | 상태 |
|---|---|---|---|
| SF-01 | 데이터 모델 (정책 노드 SO) | M | 미시작 |
| SF-02 | PolicyManager 코어 | L | 미시작 |
| SF-03 | PolicyEffectResolver (수정자) | M | 미시작 |
| SF-04 | 시스템 연동 | M | 미시작 |
| SF-05 | 정책 UI + 테스트 씬 | L | 미시작 |

---

### 방벽 확장 시스템 (wall-expansion-system)
- **GDD**: Docs/GDD/systems/wall-expansion-system.md
- **브랜치**: feature/wall-expansion-system
- **우선순위**: P1
- **상태**: GDD작성

| SF | 서브피처 | 크기 | 상태 |
|---|---|---|---|
| SF-01 | 데이터 모델 (확장 레벨 SO) | S | 미시작 |
| SF-02 | WallExpansionManager 코어 | M | 미시작 |
| SF-03 | 건설/자원 연동 + 슬롯 해금 | M | 미시작 |
| SF-04 | UI + 테스트 씬 | M | 미시작 |

---

### 희망/불만 게이지 (morale-system)
- **GDD**: 미작성
- **우선순위**: P1
- **상태**: 미시작
...
### 다음 추천 작업
1. → **탐사/원정 시스템**: P0 마지막, M1 완성에 필요
2. → **희망/불만 게이지**: GDD 미작성, `/system-design` 필요
