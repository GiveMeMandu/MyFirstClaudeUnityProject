...
| **적 AI** | **Yellow** | 12종 적의 행동 패턴 차별화 + ECS 내 상태 머신 | ECS + Burst (상태 기반 AI) | 복잡도 중간. 우선순위 기반 단순 AI라 관리 가능 |
...
| 항목 | 예산 (ms) | 비고 |
|---|---|---|
| **적 이동 + AI** | 4~6ms | 3,000개체 플로우 필드 룩업 + 이동. Burst 최적화 |
| **타워 공격 + 타겟 선택** | 2~3ms | 15~25개 타워 x 사거리 내 적 탐색 |
| **투사체 시뮬레이션** | 1~2ms | 엔티티 풀링. 동시 50~100 투사체 |
| **분대 NPC** | 1~2ms | 15~50 NPC. 적과 동일 시뮬레이션 |
| **물리/충돌** | 2~3ms | 간소화된 충돌 (ECS 기반 거리 검사, 물리 엔진 사용 최소화) |
| **렌더링** | 10~14ms | URP + Entities Graphics + GPU Instancing. 드로우 콜 100~200 목표 |
| **UI 갱신** | 1~2ms | 체력바, 미니맵, 전투 UI |
| **기타 (오디오, 파티클 등)** | 2~3ms | 오디오 믹서 + 파티클 이펙트 (LOD 적용) |
| **여유** | 3~5ms | 예측 불가 스파이크 대응 |
| **합계** | ~33ms | 30fps 프레임 예산 내 |
...
| **메시/지오메트리** | 128MB~256MB | 건물 10종 + 적 12종 + 분대 + 맵 지형 |
...
| TR-04 | **ECS-MonoBehaviour 브릿지 버그/복잡도** | 중간 | 중간 | **중간** | (1) 명확한 인터페이스 계약 (WaveDefense.md 섹션 12.1 기반), (2) 페이즈 전환 시 데이터 스냅샷 패턴, (3) 브릿지 레이어 단위 테스트 | 미조치 |
...
| TR-08 | **Entities Graphics 렌더링 한계 (적 유형별 메시 차이)** | 낮음 | 중간 | **낮음** | (1) 적 12종을 3~4개 기본 메시 + Material Override로 차별화, (2) LOD 시스템 적용, (3) 먼 거리 적은 빌보드/스프라이트 전환 | 미조치 |
...
2. **Bridge Layer를 단일 지점으로 집중**: 낮->밤 전환 시 `BattleInitializer`가 MonoBehaviour 데이터를 ECS Entity로 변환하고, 밤->낮 전환 시 `BattleResultCollector`가 ECS 결과를 MonoBehaviour 데이터로 수집한다. 이 두 클래스만이 양쪽 세계를 넘나든다.
...
| **에디터 도구** | 커스텀 에디터 윈도우 (밸런스 시뮬레이터, 웨이브 미리보기) | 밸런싱 이터레이션 가속 |
