...
- **버전**: v0.2
- **참조 문서**: Vision.md (v2.5, 섹션 5.3/5.3.1/5.3.2), Economy-Model.md (v1.1, 섹션 7/9/11), Per-Turn-Budget.md (v1.0), Construction.md (v0.1), Workforce.md (v0.1), Cross-Reference-Matrix.md (revision-4)
...
| **5. 정산** | 피해 비율에 따른 보상/피해, 분대 장비 소모, 부상 판정 | 다음 낮으로 전환 |
...
> **Vision.md와의 불일치**: Vision.md 5.3.1 속도 조절 항목에 "1x / 2x / 3x + 일시정지 가능"으로 명시되어 있으나, 인터뷰 결과 3x는 불필요로 판단하여 제거하였다. --> 섹션 13 불일치 목록 참조.
...
> **Vision.md와의 불일치**: Vision.md 5.3.1에서 명령 2종(이동/집결)으로 정의되어 있으나, 인터뷰 결과 3종(이동/공격/정지)으로 확장하였다. --> 섹션 13 불일치 목록 참조.
...
| # | 검증 항목 | 성공 기준 | 폴백 |
|---|---|---|---|
| 1 | 3,000 엔티티 동시 시뮬레이션 | 30fps 이상 (중간 사양) | 엔티티 수 감소 + 파티클/군중 셰이더로 물량감 유지 |
| 2 | 분대 명령 반응성 | 명령 입력 후 0.1초 이내 반응 | 명령 대기열 + 예측 이동 |
| 3 | 적 AI 경로 탐색 | 500+ 적 동시 경로 탐색 시 프레임 드롭 없음 | 플로우 필드 기반 경로 탐색 |
...
다음 항목은 인터뷰 결과 Vision.md의 기존 스펙에서 변경되었다. Vision.md 업데이트가 필요하다.

| # | 항목 | Vision.md 기존 | 본 GDD 변경 | 변경 근거 |
|---|---|---|---|---|
| 1 | **분대 명령 유형** | 2종: 이동/집결 (섹션 5.3.1) | **3종: 이동/공격/정지** | "집결"은 "공격"과 "정지"로 분리하는 것이 SC2 레퍼런스와 일관적이며, 전술적 선택지를 넓힘. "이동 중 교전 여부"를 플레이어가 제어 가능 |
| 2 | **속도 조절** | 1x / 2x / 3x + 일시정지 (섹션 5.3.1) | **1x / 2x + 일시정지 (3x 제거)** | 3x에서는 분대 명령 반응 시간이 지나치게 짧아 "가벼운 마이크로" 원칙이 훼손됨. 2x로 충분한 속도감 제공 |
| 3 | **방어 결과 용어** | "완벽 방어/부분 돌파/대규모 돌파/기지 붕괴" (섹션 4.2.1) | **"완벽 방어/경미 피해/중간 피해/대규모 피해"** | "피해 비율" 용어가 판정 기준과 직관적으로 일치. 게임오버 조건은 본부 파괴로 변경 |
| 4 | **완벽 방어 기준** | 피해 0 (방벽 외곽에서 적 전멸) (Economy-Model.md 7.1) | **피해 비율 ~10% 이하** (약간의 유예) | 대규모 물량전에서 피해 0은 비현실적. ~10% 유예로 완벽 방어 달성률을 합리적 수준으로 유지 |
| 5 | **방어 결과 4단계** | 3단계 + 기지 붕괴 (Vision) / Economy-Model.md 4단계 | **4단계 (완벽/경미/중간/대규모)** + 본부 파괴(게임오버) | 4단계로 경제 효과 스펙트럼을 넓힘. 게임오버 조건은 본부 파괴 |

### 13.4 Economy-Model.md 동기화 필요 사항

| 항목 | Economy-Model.md 현재 | 변경 제안 |
|---|---|---|
| 섹션 7.1 방어 결과 | "완벽 방어/부분 돌파/대규모 돌파/기지 붕괴" 4단계 | "완벽 방어/경미 피해/중간 피해/대규모 피해" 4단계 + 본부 파괴(게임오버) |
| 섹션 7.1 완벽 방어 기준 | "건물 손상 0개" | "피해 비율 ~10% 이하 (건물 0~1개 손상)" |
| 섹션 7.1 용어 통일 | "부분 돌파/대규모 돌파" | "경미 피해/중간 피해/대규모 피해" |
...
| 12 | 장비 소모 (고급/분대/전투) | 0.5~2.0 | 1.0 | 밤 전투 결과 정산 시 |
...
| 5 | 적이 기지 중심부(본부)에 도달 | 본부 HP 소진 시 게임오버. 본부만 "파괴" 가능한 유일한 건물. --> 건설 시스템 GDD (Construction.md) 섹션 3.1 참조 |
