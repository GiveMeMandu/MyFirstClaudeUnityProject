...
- **담당**: economy-designer
- **상태**: draft
- **버전**: v1.2 (방어 결과 체계 WaveDefense 동기화, 용어 통일, 참조 갱신)
- **최종 수정일**: 2026-04-03
- **참조 문서**: Vision.md v2.6, StarCraft 2(자원 구조), Diablo(보석 소켓), Frostpunk.md, Thronefall.md
...
**방어 결과 판정 기준 (WaveDefense.md 6.2 동기화):**
...
> **본부 파괴 = 게임오버**: 본부 HP가 0이 되면 피해 비율 판정과 무관하게 즉시 게임오버. 본부 파괴는 4단계 판정과 별도의 조건이다. (→ 섹션 8.1, Construction.md 섹션 3.1 참조)
...
- 일반 건물은 **파괴되지 않는다** (손상만 발생). HP가 0이 되면 손상(비활성) 상태로 전환되며, 수리비를 지불하여 복구한다. 수리는 플레이어 수동 지시 (→ 건설 시스템 GDD 참조). 용어: 일반 건물은 "손상", 본부에 대해서만 "파괴" 사용 (Vision v2.6 용어 정책)
...
| 버전 | 날짜 | 변경 내용 |
|---|---|---|
| v1.0 | 2026-04-01 | 초안 작성 (자원 3종, 인력 소켓 모델, Faucet/Sink 분석, 턴별 예산) |
| v1.1 | 2026-04-01 | 희망/불만 시스템 폐기, 영웅 유닛 모델 적용, 방벽 직선 강화, "돌파" -> "피해 비율" 용어 변경 |
| v1.2 | 2026-04-03 | 방어 결과 체계 WaveDefense.md 6.2와 동기화 (4단계 + 본부 파괴 별도 분리), "건물 파괴" -> "건물 손상" 용어 통일 (본부에만 "파괴" 유지), 완벽 방어 기준 "피해 비율 ~10% 이하"로 명시, Vision.md 참조를 v2.4 -> v2.6으로 갱신 |
